...
function init() {
    ...
    var colorStroke = "#707070"; //"#eb0e3d" //red;
    var color = ["#8dcbf5", "#deff0e", "#F4AC3E", "#8CC63E"];
    ...
    function createBall(xInit, yInit, ySpeed, color, xAlpha, yReflSpeed, dirAngle) {
        ...
        var bColor = color[Math.round(Math.random()*color.length)];
        var ball = new createjs.Shape()
            .set({ r: r, x: xInit, y: yInit, ySpeed: ySpeed, alpha: xAlpha * r / rMax, dirAngle: dirAngle });
        // var fill = ball.graphics.f("#deff0e").command; // store the fill to change it
        ball.graphics.ss(4).s("#deff0e").dc(0, 0, r);
    ...
    function generateBalls(numBalls) {
        ...
        for (var i = 0; i < numBalls; i++) {
            ...
            yReflSpeed = ySpeed * 1.15;
            var xAlpha = Math.random() * (1 - .8) + .8;
